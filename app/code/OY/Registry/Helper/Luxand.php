...
class Luxand extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function createCustomer($name, $store,$img){
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_TIMEOUT => 30,
            CURLOPT_CUSTOMREQUEST => "POST",
            CURLOPT_POSTFIELDS => [ "name" => $name, "store" => $store, "photo" => $img],
    ...
    public function getCustomerFace($customer_id){

        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => $this->getUrlRest()."/subject/".$customer_id,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
    ...
    public function addCustomerPhoto($customer_id, $img) {
        $curl = curl_init();
    ...
    public function deleteCustomerPhoto($customer_id, $photo_id) {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => $this->getUrlRest()."/subject/".$customer_id."/".$photo_id,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
